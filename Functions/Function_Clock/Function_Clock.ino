...
void loop() {
    ...
    static char clock_hour = 0, clock_min = 0, clock_sec = 0;
    static char alarm_hour = 0, alarm_min = 0, alarm_sec = 0;
    ...
    if(!digitalRead(BTN_SET)){
      if(lastStatus2 == 0){
        ...
        ssd1306_fillScreen( 0x00 );
        if(menuChoose != MENU_SHOW){
          if(++menuChoose > MENU_SET_ALARM_ON_OFF) 
            menuChoose = MENU_SET_CLOCK_HOUR;
        }
        else{
          if(buzzerOn) buzzerOn = 0;
        }
      }
    }
    else{
    ...
    if(timerExpired){
      ...
      if(timer_tenth_sec >= 10){
        ...
        if(menuChoose == MENU_SHOW){
          ...
          if((clock_sec == 0) && (clock_min == alarm_min) && (clock_hour == alarm_hour) && alarmOn){
            buzzerOn = 1;
          }
        }
      }
    ...
    switch(menuChoose){
      case MENU_SHOW:
        ...
        if(alarmOn){
          sprintf(buffer2, "%02d:%02d:%02d", alarm_hour, alarm_min, alarm_sec);
          ssd1306_printFixedN(15, 30, buffer2, STYLE_NORMAL, FONT_SIZE_2X);
          ssd1306_printFixedN(40, 50, "Alarm on", STYLE_NORMAL, FONT_SIZE_NORMAL);
        }
        else{
           ssd1306_printFixedN(38, 50, "Alarm off", STYLE_NORMAL, FONT_SIZE_NORMAL);
        }
        break;
      ...
      case MENU_SET_ALARM_HOUR:
        alarm_hour = getVoltage() >> 3;
        if(alarm_hour > 23) alarm_hour = 23;
        
        if((timer_tenth_sec%5) < 2)
          sprintf(buffer1, "%02d:%02d:%02d", alarm_hour, alarm_min, alarm_sec);
        else
          sprintf(buffer1, "  :%02d:%02d", alarm_min, alarm_sec);
          ssd1306_printFixedN(15, 35, buffer1, STYLE_NORMAL, FONT_SIZE_2X);

        ssd1306_printFixedN(10, 10, "Set Alarm", STYLE_NORMAL, FONT_SIZE_2X);
        break;
        
      case MENU_SET_ALARM_MIN:
        alarm_min = getVoltage() >> 3;
        if(alarm_min > 59) alarm_min = 59;
        
        if((timer_tenth_sec%5) < 2)
          sprintf(buffer1, "%02d:%02d:%02d", alarm_hour, alarm_min, alarm_sec);
        else
          sprintf(buffer1, "%02d:  :%02d", alarm_hour, alarm_sec);
        ssd1306_printFixedN(15, 35, buffer1, STYLE_NORMAL, FONT_SIZE_2X);
          
        ssd1306_printFixedN(10, 10, "Set Alarm", STYLE_NORMAL, FONT_SIZE_2X);
        break;
        
      case MENU_SET_ALARM_ON_OFF:
         
        if(getVoltage() > 400) {
          alarmOn = 1;
          ssd1306_printFixedN(10, 35, "Alarm ON  ", STYLE_NORMAL, FONT_SIZE_2X);
        }
        else{
          alarmOn = 0;
          ssd1306_printFixedN(10, 35, "Alarm OFF ", STYLE_NORMAL, FONT_SIZE_2X);
        }
        ssd1306_printFixedN(10, 10, "Set Alarm", STYLE_NORMAL, FONT_SIZE_2X);
